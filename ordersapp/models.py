...
class Order(models.Model):
    ...
    status = models.CharField(choices=ORDER_STATUS_CHOICES, verbose_name='статус', max_length=4, default=FORMING)
    ...
    def get_summary(self):
        items = self.orderitems.select_related()
        return {'get_total_cost': sum(list(map(lambda x: x.get_product_cost(), items))),
                'get total_quantity': sum(list(map(lambda x: x.quantity, items)))}
